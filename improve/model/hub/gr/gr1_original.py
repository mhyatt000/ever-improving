...
class GR1_original(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.embed_gripper_state = torch.nn.Linear(
            2, hidden_size
        ...
        self.action_queries = nn.Embedding(
            1, hidden_size
        )  # arm + gripper, weight from bytedance
        ...
        self.pred_gripper_act = nn.Linear(
            hidden_size // 2, 1
        ...
        self.decoder_pred = nn.Linear(
            hidden_size, self.patch_size**2 * 3, bias=True
        )  # decoder to patch
        self.decoder_pos_embed = nn.Parameter(
            torch.zeros(1, (self.image_size // self.patch_size) ** 2, hidden_size),
            requires_grad=False,
        )  # (1, n_patch, h)
    ...
    def forward(self, rgb, hand_rgb, state, language, attention_mask):
        ...
        if self.fwd_pred:
            ...
            if self.fwd_pred_hand:
                ...
                obs_hand_targets = obs_hand_targets.reshape(
                    shape=(batch_size, sequence_length, h_p * w_p, (p**2) * 3)
                )  # (b, t, n_patches, p*p*3)
                if not self.without_norm_pixel_loss:
                    # norm the target
                    obs_hand_targets = (
                        obs_hand_targets - obs_hand_targets.mean(dim=-1, keepdim=True)
                    ) / (
                        obs_hand_targets.var(dim=-1, unbiased=True, keepdim=True).sqrt()
                        + 1e-6
                    )

        # Use resampler to process patch embeddings
        patch_embeddings = patch_embeddings.unsqueeze(
            1
        )  # (b * t, 1, n_patches, patch_feat_dim)
        patch_embeddings = self.perceiver_resampler(
            patch_embeddings
        )  # (b * t, 1, n_patch_latents, patch_feat_dim)
        ...
        if self.use_hand_rgb:
            ...
            hand_patch_embeddings = hand_patch_embeddings.view(
                batch_size, sequence_length, self.n_patch_latents, self.patch_feat_dim
            )  # (b, t, n_patch_latents, patch_feat_dim)

        # Embed images and patches
        obs_embeddings = self.embed_img(obs_embeddings.float())  # (b, t, h)
        patch_embeddings = self.embed_patch(
            patch_embeddings.float()
        )  # (b, t, n_patch_latents, h)
        if self.use_hand_rgb:
            hand_obs_embeddings = self.embed_hand_img(
                hand_obs_embeddings.float()
            )  # (b, t, h)
            hand_patch_embeddings = self.embed_hand_patch(
                hand_patch_embeddings.float()
            )  # (b, t, n_patch_latents, h)
        ...
        patch_embeddings = patch_embeddings + time_embeddings.view(
            sequence_length, 1, self.hidden_size
        )
        ...
        if self.act_pred:
            ...
            action_chunk_queries = (
                self.action_chunk_queries.weight + action_queries
            )  # (chunk_size, h)
            ...
            stacked_inputs = torch.cat(
                (stacked_inputs, action_chunk_queries), dim=2
            )  # (b, t, n_tokens, h)
        if self.fwd_pred:
        ...
        stacked_inputs = stacked_inputs.reshape(
            batch_size, n_tokens * sequence_length, self.hidden_size
        )
        ...
        if self.act_pred:
            act_query_attention_mask = torch.zeros(
                (batch_size, sequence_length, n_act_pred_tokens),
                dtype=torch.long,
        ...
        if self.fwd_pred:
            obs_query_attention_mask = torch.zeros(
                (batch_size, sequence_length, n_patch_tokens + n_obs_tokens),
                dtype=torch.long,
                device=stacked_inputs.device,
            )
            stacked_attention_mask = torch.cat(
        ...
        stacked_attention_mask = stacked_attention_mask.reshape(
            batch_size, n_tokens * sequence_length
        )
        ...
        if self.act_pred:
            action_embedding = x[
                :,
                :,
                act_query_token_start_i : (act_query_token_start_i + self.chunk_size),
            ]
