...
class Dataset(object):
    """
    A dataset that stores observations, actions, rewards, masks, dones_float, and next_observations.

    sample(batch_size)
        returns a Batch object with batch_size samples.
    get_initial_states()
        returns the initial states of each trajectory.
    get_monte_carlo_returns(discount)
        returns the Monte Carlo returns of each trajectory.
    take_top(percentile)
        keeps the top percentile trajectories.
    take_random(percentage)
        keeps a random percentage of the trajectories.
    train_validation_split(train_fraction)
