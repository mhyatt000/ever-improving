...
def main(_):
    ...
    ds = dm_control_suite.dataset(
        root_path=FLAGS.path,
    ...
    for i, sample in tqdm(enumerate(ds)):
        obs = gym.spaces.flatten(env.observation_space, sample.data[0]).astype(
            np.float32
        )
        action = gym.spaces.flatten(env.action_space, sample.data[1]).astype(np.float32)
        reward = float(sample.data[2].numpy().item())
        mask = float(sample.data[3].numpy().item())
        next_obs = gym.spaces.flatten(env.observation_space, sample.data[4]).astype(
            np.float32
        )
    ...
    for i in tqdm(range(len(ordered_observations))):
        if (
            i == len(ordered_observations) - 1
            or np.linalg.norm(
                ordered_observations[i + 1] - ordered_next_observations[i]
            )
            > 1e-5
        ):
