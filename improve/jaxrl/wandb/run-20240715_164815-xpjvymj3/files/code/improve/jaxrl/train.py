...
class Job:
    ...
    log_dir: Optional[str] = None  # Log directory.
...
def main(cfg: MyConfig):
    ...
    algo = cfg.algo.name
    # run_name = f"{cfg.env_name}__{cfg.algo.name}__{cfg.job.seed}__{int(time.time())}"
    ...
    if cfg.job.wandb:
        ...
        wandb.init(
            ...
            name=None,  # get unique id
            monitor_gym=True,
            save_code=True,
        )
        cfg.job.log_dir = osp.join(cfg.job.log_path, wandb.run.name)
    ...
    np.random.seed(cfg.job.seed)
    random.seed(cfg.job.seed)
    ...
    if algo.name == "sac":
        agent = SACLearner(cfg.job.seed, obs, act, **agent_kwargs)
    elif algo.name == "awac":
        agent = AWACLearner(cfg.job.seed, obs, act, **agent_kwargs)
    else:
    ...
    for i in tqdm(range(1, cfg.max_steps + 1), smoothing=0.1, disable=not cfg.job.tqdm):
        ...
        if done:
            ...
            for k, v in info["episode"].items():
                logger.record(f"training/{k}", v, info["total"]["timesteps"])
        ...
        if i % cfg.eval_interval == 0:
            ...
            for k, v in eval_stats.items():
                logger.record(f"evaluation/average_{k}s", v, info["total"]["timesteps"])
            logger.dump(step=i)

            eval_returns.append((info["total"]["timesteps"], eval_stats["return"]))
            np.savetxt(
                os.path.join(cfg.job.log_dir, f"{cfg.job.seed}.txt"),
                eval_returns,
                fmt=["%d", "%.1f"],
            )
