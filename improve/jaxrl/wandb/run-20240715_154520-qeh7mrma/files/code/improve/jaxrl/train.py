...
from improve.jaxrl.utils import make_env
...
def main(cfg: MyConfig):
    ...
    summary_writer = SummaryWriter(os.path.join(cfg.job.save_dir, run_name))

    if cfg.job.save_video:
        video_train_folder = os.path.join(cfg.job.save_dir, "video", "train")
        video_eval_folder = os.path.join(cfg.job.save_dir, "video", "eval")
    else:
        video_train_folder = None
        video_eval_folder = None

    env = make_env(cfg.env_name, cfg.seed, video_train_folder)
    eval_env = make_env(cfg.env_name, cfg.seed + 42, video_eval_folder)
    ...
    for i in tqdm(range(1, cfg.max_steps + 1), smoothing=0.1, disable=not cfg.job.tqdm):
        ...
        if i % cfg.eval_interval == 0:
            ...
            np.savetxt(
                os.path.join(cfg.job.save_dir, f"{cfg.seed}.txt"),
                eval_returns,
                fmt=["%d", "%.1f"],
            )
