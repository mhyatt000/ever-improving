...
class FrameStack(gym.Wrapper):
    """
    Stack `num_stack` consecutive frames along the last dimension. For instance,
    if `num_stack` is 4, the shape of the observation will be `(H, W, C*4)`.

    WARN: only works for gym.spaces.Box observation space.
    """
    ...
    def step(self, action):
        """ appends obs to the _stack, and returns the stacked obs """
