...
file_path = "improve/wrapper/infos.json"
...
for position in ["source", "target"]:
    ...
    for i in range(7):
        axs[i].hist([pos_dict[position][j][i] for j in range(len(pos_dict[position])) if not successes[j]], color="orange", alpha=0.5)
        axs[i].hist([pos_dict[position][j][i] for j in range(len(pos_dict[position])) if successes[j]], alpha=0.5)
